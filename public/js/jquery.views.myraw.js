...
function linkedView( view ) {
    ...
    view.updateViews = view_updateViews;
...
function view_updateViews( index, newItem, oldItem, tmpl ) {
  var self = this,
      context = self.ctx,
      views = self.views;
  ...
  if (newItem && oldItem) {
    self.removeViews.call(self, index );
    self.addViews.call(self, index, newItem, tmpl);
    if ( self.ctx.afterUpdate ) {
      self.ctx.afterUpdate.call(self.views[ index ], oldItem, newItem); 
    }
  }
...
function view_onDataChanged( eventArgs ) {
	if ( eventArgs ) {
		...
		switch ( action ) {
			...
			break;
      case "update":
        self.updateViews( index, eventArgs.newItem, eventArgs.oldItem );
      break;
			case "move":
